...
class PgNode_a(PgNode):
    ...
    def __init__(self, action: Action):#q
...
class PlanningGraph():
    ...
    def update_a_mutex(self, nodeset):
        ...
        :param nodeset: set of PgNode_ax (siblings in the same level)
        :return:
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        for effect in a1.effect_rem:
            if effect in a2.precond_pos:
                return True
        for effect in a2.effect_rem:
            if effect in a1.precond_pos:
                return True
        return False
    ...
    def h_levelsum(self) -> int:
        ...
        for i in range(len(self.s_levels)):
            temp.append([x.literal for x in self.s_levels[i]])

        for g in self.problem.goal:
            for j in range(len(self.s_levels)):
                if g in temp[j]:
                    level_sum += j
                    break
